...
{ // Vectorized GF(p^2) addition and subtraction, c = 2a-b in GF((2^127-1)^2)       
...
void ecc_precomp(vpoint_extproj_t P, vpoint_extproj_precomp_t *T, v2elm_t d)
{ // Generation of the precomputation table used by the variable-base scalar multiplication ecc_mul().
  ...
  // Precomputed points use the representation (X+Y,Y-X,2Z,2dT) corresponding to (X:Y:Z:T) in extended twisted Edwards coordinates
    ...
    R1_to_R2(P, T[0], d);                  // Converting from (X,Y,Z,Ta,Tb) to (X+Y,Y-X,2Z,2dT)
    ...
    R1_to_R2(PP, T[1], d);                 // Converting from (X,Y,Z,Ta,Tb) to (X+Y,Y-X,2Z,2dT)
...
void cofactor_clearing(vpoint_extproj_t P, v2elm_t d)
{ // Co-factor clearing
  // Input: P = (X1,Y1,Z1,Ta,Tb), where T1 = Ta*Tb, corresponding to (X1:Y1:Z1:T1) in extended twisted Edwards coordinates.
  //        Curve parameter d.
  ...
  //         corresponding to (Xfinal:Yfinal:Zfinal:Tfinal) in extended twisted Edwards coordinates
    ...
    R1_to_R2(P, Q, d);         // Converting from (X,Y,Z,Ta,Tb) to (X+Y,Y-X,2Z,2dT)
...
{ // Variable-base scalar multiplication Q = k*P using a 4-dimensional decomposition
  ...
  // This function performs point validation and (if selected) cofactor clearing.
    ...
    point_setup(P, R);                                        // Clear most significant bit of input coordinates and convert to vectorized representation (X,Y,1,Ta,Tb)
    from_std_to_ext((felm_t*)&curve->d, d);
    
    if (ecc_point_validate(R, d) == false) {                  // Check if point lies on the curve
        return false;
    }
    
    decompose((uint64_t*)k, scalars);                         // Scalar decomposition
    if (clear_cofactor == true) {
        cofactor_clearing(R, d);
    }
    recode(scalars, digits, sign_masks);                      // Scalar recoding
    ecc_precomp(R, Table, d);                                 // Precomputation
...
void R1_to_R2(vpoint_extproj_t P, vpoint_extproj_precomp_t Q, v2elm_t d) 
...
{ // Modular correction of input coordinates and conversion to vectorized representation (X,Y,Z,Ta,Tb) 
...
bool ecc_point_validate(vpoint_extproj_t P, v2elm_t d)
...
{ // Fixed-base scalar multiplication Q = k*P, where the base point P is passed through P_table which contains multiples of P.
  ...
  // with (bitlength(order)+w*v) digits.
    ...
    vpoint_t A;
    vpoint_extproj_t R;
    vpoint_precomp_t S, *P_table = (vpoint_precomp_t*)P_Table;
    int i, ii;
...
{ // Allocates memory dynamically for precomputation table "T_fixed" used during fixed-base scalar multiplications.
  // This function must be called before using ecc_precomp_fixed(), which generates a precomputed table with v*2(w-1) points.
    ...
    return (digit_t*)calloc(NPOINTS_FIXEDBASE, sizeof(vpoint_precomp_t));
}
...
{ // Generation of the precomputation table used by the fixed-base scalar multiplication function ecc_mul_fixed(). 
  ...
  // This function performs point validation and (if selected) cofactor clearing.
    vpoint_t S;
    vpoint_precomp_t* Table = (vpoint_precomp_t*)table; 
    vpoint_extproj_t A, R, base[W_FIXEDBASE]; 
    vpoint_extproj_precomp_t baseb[W_FIXEDBASE], RR, T[NPOINTS_FIXEDBASE];
    unsigned int i, j, k, index, w = W_FIXEDBASE, v = V_FIXEDBASE, d = D_FIXEDBASE, e = E_FIXEDBASE;;
    unsigned long index_group;
    v2elm_t d_curve;
    
    point_setup(P, A);                                        // Clear most significant bit of input coordinates and convert to vectorized representation (X,Y,1,Ta,Tb)
    from_std_to_ext((felm_t*)&curve->d, d_curve);

    if (ecc_point_validate(A, d_curve) == false) {            // Check if point lies on the curve
        return false;
    }

    if (clear_cofactor == true) {
        cofactor_clearing(A, d_curve);
        eccnorm(A, S);
        from_ext_to_std(S->x, P->x);
        from_ext_to_std(S->y, P->y); 
        point_setup(P, A);
    }  
    ...
    for (i = 0; i < (w-1); i++) {
        ...
        R1_to_R2(base[i], baseb[i], d_curve);                 // baseb in representation (X+Y,Y-X,2Z,2dT)
        for (j = 0; j < d; j++) eccdouble(base[i+1]);         // base[i+1] = 2^d*base[i+1] using representations (X,Y,Z,Ta,Tb) <- 2*(X,Y,Z)
    }

    R1_to_R2(base[w-1], baseb[w-1], d_curve);                 // baseb in representation (X+Y,Y-X,2Z,2dT)        
    v2copy1271(A->x, T[0]->xy);                               // T[0] = A in (x,y)
    v2copy1271(A->y, T[0]->yx); 
    ...
    for (i = 0; i < (w-1); i++)                               // T[index] = (1 + u_0.2^d + ... + u_{w-2}.2^((w-1)d)) A
    {
        ...
        {
            v2add1271(T[j]->xy, T[j]->yx, RR->xy); 
            v2sub1271(T[j]->yx, T[j]->xy, RR->yx); 
            v2zero1271(RR->z2); RR->z2[0] = 1;                           
            v2mul1271(T[j]->xy, T[j]->yx, RR->t2);            // RR in representation (X+Y,Y-X,1,T)
            ...
            from_ext_to_std(S->y, P->y); 
            point_setup(P, R);
            v2copy1271(R->x, T[index]->xy);                             
            v2copy1271(R->y, T[index]->yx);                   // Precomputed points T[] in (x,y)
        }
    ...
    for (i = 0; i < (v-1); i++)                               // T[index] = 2^(ev) (1 + u_0.2^d + ... + u_{w-2}.2^((w-1)d)) A
    {
        for (j = 0; j < index; j++)
        {
            v2copy1271(T[i*index + j]->xy, R->x);                             
            v2copy1271(T[i*index + j]->yx, R->y); 
            v2zero1271(R->z); R->z[0] = 1; 
            for (k = 0; k < e; k++) eccdouble(R);             // 2^(ev) * x * A using representations (X,Y,Z,Ta,Tb) <- 2*(X,Y,Z)   
            eccnorm(R, S);                                    // R in (x,y,1,ta,tb)
            from_ext_to_std(S->x, P->x);
            from_ext_to_std(S->y, P->y); 
            point_setup(P, R);
            v2copy1271(R->x, T[(i+1)*index + j]->xy);                             
            v2copy1271(R->y, T[(i+1)*index + j]->yx);         // Precomputed points T[] in (x,y)
        }
    }

    for (i = 0; i < NPOINTS_FIXEDBASE; i++)
    {
        v2mul1271(T[i]->xy, T[i]->yx, Table[i]->xy);          // Precomputed points T[] in coordinates (x+y,y-x,2dt)
        v2mul1271(d_curve, Table[i]->xy, Table[i]->t2);
        v2add1271(Table[i]->t2, Table[i]->t2, Table[i]->t2);
        v2add1271(T[i]->xy, T[i]->yx, Table[i]->xy);
        v2sub1271(T[i]->yx, T[i]->xy, Table[i]->yx); 
        v2mod1271(Table[i]->xy, Table[i]->xy); 
        v2mod1271(Table[i]->yx, Table[i]->yx); 
        v2mod1271(Table[i]->t2, Table[i]->t2); 
    }
...
{ // Double scalar multiplication R = k*P + l*Q, where the base point P is passed through P_Table which contains multiples of P, Phi(P), Psi(P) and Phi(Psi(P)).
  ...
  // The function uses wNAF with interleaving.
    ...
    vpoint_t A;
    vpoint_precomp_t V, *P_table = (vpoint_precomp_t*)P_Table;
    ...
    if (ecc_point_validate(Q1, d_curve) == false) {            // Check if point lies on the curve
        return false;
    }
...
void ecc_precomp_double_internal(vpoint_extproj_t P, vpoint_extproj_precomp_t* Table, unsigned int npoints, PCurveStruct curve)
{ // Generation of the precomputation table used internally by the double scalar multiplication function ecc_mul_double().  
  ...
  // Output: Table containing multiples of the base point P using representation (X+Y,Y-X,2Z,2dT).
    ...
    from_std_to_ext((felm_t*)&curve->d, d_curve);    
    R1_to_R2(P, Table[0], d_curve);            // Precomputed point Table[0] = P in coordinates (X+Y,Y-X,2Z,2dT)
    ...
    for (i = 1; i < npoints; i++) {
        ...
        R1_to_R2(Q, Table[i], d_curve);        // Converting from (X,Y,Z,Ta,Tb) to (X+Y,Y-X,2Z,2dT)
    }
...
{ // Allocates memory dynamically for the precomputation table used during double scalar multiplications.
  // This function must be called before using ecc_precomp_double(), which generates a precomputed table with NPOINTS_DOUBLEMUL_WP points.
    ...
    return (digit_t*)calloc(4*NPOINTS_DOUBLEMUL_WP, sizeof(vpoint_precomp_t));
}
...
{ // Generation of the precomputation table used by the double scalar multiplication function ecc_mul_double(). 
  ...
  // This function performs point validation.
    vpoint_precomp_t *Table = (vpoint_precomp_t*)table;
    vpoint_extproj_t P1, P2, P3, P4;
    vpoint_extproj_precomp_t T[4*NPOINTS_DOUBLEMUL_WP]; 
    unsigned int i;
    v2elm_t d_curve;
    ...
    if (ecc_point_validate(P1, d_curve) == false) {      // Check if point lies on the curve
        return false;
    }
    ...
    for (i = 0; i < 4*NPOINTS_DOUBLEMUL_WP; i++) {
        v2div1271(T[i]->z2);
        v2inv1271(T[i]->z2);
        v2mul1271(T[i]->xy, T[i]->z2, Table[i]->xy);
        v2mul1271(T[i]->yx, T[i]->z2, Table[i]->yx);
        v2mul1271(T[i]->t2, T[i]->z2, Table[i]->t2); 
        v2mod1271(Table[i]->xy, Table[i]->xy); 
        v2mod1271(Table[i]->yx, Table[i]->yx); 
        v2mod1271(Table[i]->t2, Table[i]->t2); 
    }
