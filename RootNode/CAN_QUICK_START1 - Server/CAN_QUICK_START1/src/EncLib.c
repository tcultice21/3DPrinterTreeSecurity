...
// TODO: Talk to Joseph about how to keep track of Num Nodes
#define numNodes 2
...
int authToParent(void * nodes, struct can_module* can_instance) {
	...
	*/
	...
	for(int i = 0; i < RESPONSE_SIZE; i++) {
		// Change this to something related to the hardware ID, this emulates the "PUF" design
		selfData.secret_key[i] = self.addr & 0x8 + i;
	}
	...
	photon128(selfData.secret_key,RESPONSE_SIZE,message.data);
	message.header.cmd = NODE_CMD_AUTH_ENC;
	message.header.counter = self.addr;
	message.header.TTL = 1;
	message.len = RESPONSE_SIZE;
	...
	// TODO, should we encrypt this to validate that the OK was actually sent?
...
int authChildren(void * nodes, struct can_module * can_instance) {
